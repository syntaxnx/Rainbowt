...
then
	...
	then
		...
		cd /$HOME
		figlet PRDX
		echo Agora seu Termux está de volta ao original
...
then
	...
	read uname
	echo "Este é seu nome de usuário?: $uname"
	echo "$uname" > .user.cfg
	echo "1" >> .user.cfg
	clear
#Rename Username
else
	printf "Deseja alterar seu nome de usuário [Y/N]: "
	read ink
	case "$ink" in
		[yY][eE][sS]|[yY])
	rm .user.cfg;
...
updatedw() {
	...
	then
		...
		if [[ \$var == *\"up to date\"* ]];
		then 
			clear && echo \"Já rodando a última versão!!\" && echo ------------------------------------- && figlet \$(sed '1q;d' 'ver.cfg') && figlet PRDX
		else
			git pull
			bash setup.sh;
			clear && echo \"Update Success\" && echo -------------- && figlet Success && figlet \$(sed '1q;d' 'ver.cfg') && figlet PRDX && echo Reinicie para aplicar as alterações
		fi
	else
...
echo Reinicie para aplicar as alterações